...
export default function WelcomeScreen() {
  ...
  useEffect(() => {
    // Splash screen animation: fade+scale in, hold, then fade out
    Animated.sequence([
      Animated.parallel([
      ...
      Animated.delay(1200),
      Animated.timing(splashExit, { toValue: 0, duration: 500, useNativeDriver: true }),
    ]).start(() => {
      setShowSplash(false);
      // Then animate role selection in
      Animated.sequence([
        Animated.parallel([
  ...
  }, [fadeAnim, slideAnim, cardAnims, splashFade, splashScale, splashExit]);

  const handleRoleSelect = (roleKey: (typeof roles)[number]["key"]) => {
    router.push({ pathname: "/sign-in", params: { role: roleKey } });
  };

  if (showSplash) {
    return (
      <Animated.View style={[styles.splashContainer, { opacity: splashExit }]}>
        <LinearGradient colors={["#0D0D0D", "#141414", "#0D0D0D"]} style={StyleSheet.absoluteFill} />
        <Animated.View style={[styles.splashContent, { opacity: splashFade, transform: [{ scale: splashScale }] }]}>
          <View style={styles.splashLogoBadge}>
            <UtensilsCrossed size={48} color={Colors.dark.black} />
          </View>
          <Text style={styles.splashTitle}>MealMatch</Text>
          <Text style={styles.splashSubtitle}>Connecting surplus food{"\n"}with those who need it</Text>
        </Animated.View>
      </Animated.View>
    );
  }
...
const styles = StyleSheet.create({
  ...
  splashContent: { alignItems: "center", paddingHorizontal: 24 },
  splashLogoBadge: {
    width: 100,
    height: 100,
    borderRadius: 28,
